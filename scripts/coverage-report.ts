...
const report = {
  ...
  modes: modeConfigs.map((mode) => {
    ...
    return {
      ...
      playableTeams: validation?.playableTeams ?? [],
      playableRolls: validation?.playableRolls ?? [],
      incompleteTeams: validation?.incompleteTeams ?? [],
      rollCoverage: validation?.rollCoverage ?? [],
